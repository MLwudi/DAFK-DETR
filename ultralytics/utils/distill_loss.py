...
class CWDLoss(nn.Module):
    def __init__(self, channels_s, channels_t, tau=1.0, 
        ...
        self.tau = tau
        self.lambda_stat = lambda_stat  # 通道统计损失权重
        self.lambda_channel = lambda_channel  # 通道距离损失权重
    ...
    def forward(self, y_s, y_t):
        ...
        for s, t in zip(y_s, y_t):
            ...
            # 3. 通道余弦相似度损失（直接度量通道特征差异）
            s_channel = s.view(N, C, -1).mean(dim=2)  # [N, C] 通道全局描述
            t_channel = t.view(N, C, -1).mean(dim=2)
            
            s_norm = F.normalize(s_channel, dim=1)  # [N, C] 归一化通道特征
            t_norm = F.normalize(t_channel, dim=1)
            cos_sim = torch.sum(s_norm * t_norm, dim=1).mean()  # 平均余弦相似度
            cos_loss = 1 - cos_sim  # 相似度越高，损失越小
            channel_loss = cos_loss  # 仅保留余弦损失
        ...
        return total_loss / len(y_s)  # 平均各层损失
